...
public class PoiControlInduction implements HBAction, HBReset {
    ...
    public void action(HB hb) {
        ...
        new GyroscopeListener(hb) {
            ...
            public void sensorUpdated(float pitch, float roll, float yaw) {
                    ...
                    if (Math.abs(average_gyro) < FOV_GYRO_THRESHOLD) {
                        ...
                        if (Math.abs(average_gyro) > FOV_GYRO_THRESHOLD) {
                            ...
                            if (delay > FOV_WAIT_TIME) {
                                ...
                                double new_fov = changeFieldOfView(average_gyro < 0);

                                if (current_fov != new_fov) {
                                    fieldOfViewControl.setValue(new_fov);
                                    playBuzz(average_gyro > 0);
                                    if (!lowResolutionMode()) {
                ...
                double averageGyro = Math.sqrt(pitch * pitch + roll * roll);
                ...
                //if (!adjustFov)
                    ...
                    if (Math.abs(pitch) > MOVEMENT_GYRO_THRESHOLD && !lowResolutionMode()) {
                        // we need to send left right
                        upSender.setValue(pitch / 2);
                    }
            ...
            public void sensorUpdated(float x_val, float y_val, float z_val) {
                ...
                /******** Write your code below this line ********/
                    ...
                    if (lowResolutionMode()) //&& altitudeMovementValid)
                    {
                        ...
                        float scaled_val = Sensor.scaleValue(-1, 1, MIN_ALT, MAX_ALT, z_val);
