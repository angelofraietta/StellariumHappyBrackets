...
public class StellariumSlaveDriver implements HBAction, HBReset {
...
/*** End DynamicControl requestCoordinates code ***/
        ...
        Clock slaveDriver = hb.createClock(200).addClockTickListener((offset, this_clock) -> {
            /*** Write your Clock tick event code below this line ***/
            double next_value = accelerometerSimulator.getValue() + 0.01;

            if (next_value > 1){
                next_value = -1;
            }

            float scaled_val = Sensor.scaleValue(-1, 1, MIN_ACCEL_ALT, MAX_ACCEL_ALT, next_value);

            azimuthControl.setValue(scaled_val);

            accelerometerSimulator.setValue(next_value);
